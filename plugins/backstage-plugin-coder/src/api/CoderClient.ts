...
import * as coderApi from 'coder/site/src/api/api';
...
export class CoderClient implements CoderClientApi {
  ...
  constructor(
    ...
    axios.interceptors.request.use(async config => {
      ...
      // TODO: this is not right, needs to return token string
      config.headers['Coder-Session-Token'] = await this.getRequestInit();
      return config;
    });
  ...
  getWorkspaces = async (coderQuery: string): Promise<readonly Workspace[]> => {
    ...
    const response = await this.api.getWorkspaces({ q: coderQuery });

    const withRemappedImgUrls = response.workspaces.map(ws => {
